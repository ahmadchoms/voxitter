...
export const postsService = {
  async getPosts(offset = 0, limit = 25, userId) {
    try {
      ...
      const processedData = data?.map((post) => ({
        ...post,
        is_liked: post.likes?.some((like) => like.user_id === userId) || false,
        is_bookmarked: post.bookmarks?.some((bm) => bm.user_id === userId) || false,
  ...
  async getPostById(id, userId) {
    try {
      ...
      const processedData = {
        ...
        is_bookmarked: data.bookmarks?.some((bm) => bm.user_id === userId) || false,
  ...
  async getBookmarkedPosts(offset = 0, limit = 25, userId) {
    try {
      const { data, error } = await supabase
        .from("bookmarks")
        .select(
      ...
      const processedData =
        data?.map((bookmark) => ({
          ...bookmark.post,
          is_bookmarked: true,
          created_at: new Date(bookmark.post.created_at).toLocaleDateString("id-ID"),
        })) || [];
